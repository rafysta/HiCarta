...
カタログ URL・リモートファイルの読み込み方式・IGV のゲノム ID・表示言語などの既定値は、アプリと同じフォルダの `config.txt` に保存されます。アプリ内の **設定 → 設定ファイルを編集…** から編集できます（詳細は **[画面と操作の説明](interface.md)** を参照）。手動で編集する場合は、`config.example.txt` を `config.txt` にコピーして書き換えます。
