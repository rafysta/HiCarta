...
保存したブックマークは一覧に並び、クリックするといつでもその領域に戻れます。記録されるのは**場所だけ**なので、別のサンプルを開いた状態でクリックすれば、そのサンプルの同じ領域へ移動します（注目領域をサンプル間で見比べるための使い方です）。正規化・解像度・カラースケールは今の設定のまま変わりません。不要になったら **削除** で消せます。**Excel に保存** / **Excel から読み込み（追加）** で、一覧を注釈・共有できる `.xlsx` として入出力できます（[詳細](data-catalog.md#ブックマークの-excel-入出力)）。
