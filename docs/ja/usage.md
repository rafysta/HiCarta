...
1. **データ** ボタン → **トラック** タブを開きます。
2. **bigWig/BED ファイル/URL** に bigWig のパスまたは URL を入れます（**参照…** も可）。
3. **種類** を「bigWig」にして、**トラックを追加** をクリックします。
...
1. **データ** ボタン → **トラック** タブを開きます。
2. ファイル欄に Border Strength ファイル（`*_BS.txt`）のパスまたは URL を入れます。
3. **種類** を「境界強度（Border Strength）」にして、**トラックを追加** をクリックします。
