...
Juicer 形式の `.hic` を直接読み込めます。サンプルは **[データカタログ](data-catalog.md)**（サンプルを一覧にした Excel ファイル）から選んで開きます。正規化や解像度はサンプルごとに事前設定でき、開いた後に切り替えることもできます。

→ [Hi-C マップを読み込む](usage.md#load-hic)

### ゲノム全体を一度に見る

縦横の両軸に全染色体を配置し、すべての染色体の組み合わせを一度に表示できます。対角線上には各染色体の cis マップ、その外側のブロックには染色体間マップが並びます。軸には染色体名が入り、境界には罫線が引かれます。そのまま任意のブロックにズームインできます。

→ [マップ上を移動する](interface.md#navigate)

### 染色体間（trans）の相互作用を見る

同じ染色体どうしの通常のマップに加えて、縦軸に*別の*染色体を指定して、2 本の染色体間の相互作用を表示できます。マップは長方形になりますが、移動・ズーム・解像度の自動切り替え・カラースケール・ブックマーク・出力は、通常のマップとまったく同じように使えます。トラックは横軸に付いたままです。
...
注目している領域に名前を付けて保存し、あとからワンクリックで同じ場所に戻れます。記録されるのは場所だけなので、サンプルを切り替えてから同じブックマークをクリックすれば、そのサンプルの同じ領域が見られます。注目領域をサンプル間で見比べるのに便利です。
