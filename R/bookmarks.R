...
#
# A bookmark stores WHERE to look (chr, x/y range) and NOTHING about the data.
# It is a region of interest, meant to be visited in one sample after another,
# so jumping to one moves inside whatever is currently open (app.R, bm_goto).
# Import only ever APPENDS to the current list (per spec).
#
# xlsx columns: bookmark_name, chr, start, end, chr_y, ystart, yend, comment
# Older files also carry catalog_id / path / entry / norm / resolution / vmax;
# those columns are simply ignored, so old bookmark sheets still load.
...
bookmarks_to_df <- function(bms) {
  empty <- data.frame(bookmark_name = character(0),
